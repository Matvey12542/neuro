...
$(function () {
  ...
  // Ajax send data to neural net (php) and get result.
  $("#neuro-go-button").click(function () {
    $.ajax({
      type: "POST",
      url: "start.php",
      ...
      success: function(result) {
        ...
        for (x = 0; x < result.length; x++) {
          var clas = result[x] == max ? 'winner' : '';
          markup += "<div class='" + clas + "'>" + x + '  ======>>>  ' + result[x] + "</div>";
        }
  ...
  $(".train-button").click(function () {
    var json_data = getTableData();
    var right_result = parseInt($(this).val(), 10);
    if (!Number.isInteger(right_result)) {
      alert('Please enter right result');
      return;
    }
    ...
    $.ajax({
      type: "POST",
      url: "add_example.php",
      data: {'data': json_data, 'right_result': right_result},
      dataType: 'json',
      success: function(result) {
  ...
  // Train network.
  $("#start-train-button").click(function () {
    $.ajax({
      type: "GET",
      url: "train.php",
      success: function(result) {
  ...
  function getTableData() {
    ...
    $('#drawing-table tr td').each(function (index1) {
      var $this = $(this);
      data[index1] = ($this.hasClass('filled')) ? 1 : 0;
    });
